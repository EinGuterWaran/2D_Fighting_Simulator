...
def simulate_game(map, fighters, frames_per_second):
    ...
    while not done:
        ...
        if len(fighters) == 1:
            ...
            for x in stats:
                ...
                the_text2 = x["name"] + ", KILLS: " + str(
                    x["kills"]) + ", DAMAGE: " + str(
                        x["damage"]) + ", HITS: " + str(
                            x["hits"]) + ", CRITIAL HITS: " + str(x["crit"])
                print(the_text2)
        ...
        #When all are dead at the end of the round
        if len(fighters) == 0:
            max = None
            winner = []
            for deadFig in died:
                if max == None:
                    max = deadFig.temp
                    winner.append(deadFig)
                elif deadFig.temp == max:
                    winner.append(deadFig)
                elif deadFig.temp > max:
                    winner.clear()
                    winner.append(deadFig)
            ...
            if len(winner) == 1:
                ...
                if len(winner) == 2:
                    ...
                    howMany = str(len(winner))
                the_text = "WOW! What a bizarre situation. " + howMany + " fighters are knocked out at the same time and come back to their senses at the same time. This is a REAL draw between:"
                text = font.render(the_text, True, GREEN)
            print(the_text)
            textPipe.append(the_text)
            if (len(winner) != 1):
...
def main():
    while True:
        print(
            "Enter e1 if you want to see example game #1 and e2 for example game #2. If you want to set up your own game enter o!"
        )
        mode = input("")
    ...
    if mode == "e1":
        ...
        while True:
            mapx = input("Map width (8 or higher): ")
            if str(mapx).isnumeric() and int(mapx) >= 8:
                break
        ...
        while True:
            ...
            while True:
                    ...
                    "Enter y if you want to add another player. If not, enter n! "
                )
                if end == "y" or end == "n":
                    break
